...
import com.shawn.lib.action_refresh.sdk.Compat;
import com.shawn.lib.smooth_progress.SmoothProgressBar;
...
public class DefaultHeaderTransformer extends HeaderTransformer {
    ...
    private void setupViewsFromStyles(Activity activity, View headerView) {
        ...
        Drawable bg = styleAttrs.hasValue(R.styleable.PullToRefreshHeader_ptrActionHeaderBackground)
                ? styleAttrs.getDrawable(R.styleable.PullToRefreshHeader_ptrActionHeaderBackground)
		...
		final int titleTextColor = styleAttrs.getColor(
				R.styleable.PullToRefreshHeader_ptrHeaderTitleTextColor,
				getTextColor(abContext));
		if (titleTextColor != 0) {
			mHeaderTextView
					.setTextColor(ColorStateList.valueOf(titleTextColor));
		}
		final float titleTextSize = styleAttrs.getDimensionPixelSize(
				R.styleable.PullToRefreshHeader_ptrHeaderTitleTextSize,
				getTextSize(abContext));
		if (titleTextSize > 0) {
			mHeaderTextView.setTextSize(TypedValue.COMPLEX_UNIT_PX, titleTextSize);
		}
