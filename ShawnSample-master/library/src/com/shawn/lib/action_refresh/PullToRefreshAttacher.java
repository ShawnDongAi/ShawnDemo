...
import com.shawn.lib.action_refresh.listeners.HeaderViewListener;
...
public class PullToRefreshAttacher {
	...
	final boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_MOVE: {
			...
			if (mIsBeingDragged && y != mLastMotionY) {
				...
				if (y >= mPullBeginY) {
					...
					if (y - mPullBeginY - 10 > getScrollNeededForRefresh(mViewBeingDragged)) {
						mPullBeginY = y
								- getScrollNeededForRefresh(mViewBeingDragged);
					}
		...
		case MotionEvent.ACTION_CANCEL:
		case MotionEvent.ACTION_UP: {
			if (event.getY() - mPullBeginY < getScrollNeededForRefresh(mViewBeingDragged)) {
